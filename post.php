...
<div class="page-wrapper">
    ...
    <div class="cover">
        <div class="cover-image" style="background-image:url(<?php $this->fields->previewImage(); ?>)">
            ...
            <div class="meta"><?php array_map(function($v){echo '<a class="category" onclick="showArchive(\''.$v['permalink'].'\')" style="'.$v['description'].'">'.$v['name'].'</a>';},$this->categories) ?> <?php $this->author(); ?> · <?php echo time_elapsed_string('@'.$this->created) ?>前 · 阅读: <?php $this->viewsNum();?></div></div>
        </div>
    </div>
    <?php else: ?>
    <div class="title"><?php $this->title(); ?>
    <div class="meta"><?php array_map(function($v){echo '<a class="category" onclick="showArchive(\''.$v['permalink'].'\')" style="'.$v['description'].'">'.$v['name'].'</a>';},$this->categories) ?> <?php $this->author(); ?> · <?php echo time_elapsed_string('@'.$this->created) ?>前 · 阅读: <?php $this->viewsNum();?></div>
    </div>
    <?php endif; ?>
    <div class="content">
        <?php $this->content(); ?>
